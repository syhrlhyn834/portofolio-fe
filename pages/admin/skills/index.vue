<template>
  <div>
    ...
    <h1 class="text-2xl font-semibold mb-4">Skills</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="skill in filteredSkill" :key="skill.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            <td class="px-12 py-4">{{ skill.id }}</td> <!-- Adjusted padding -->
            <!-- Displaying Skill Image -->
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              <img :src="skill.image" alt="Skill Image" class="h-12 w-12 object-cover rounded-md sm:h-12 sm:w-12">
            </td>



            <!-- Displaying Skill Description -->
            <td class="px-12 py-4">{{ skill.description }}</td> <!-- Adjusted padding -->
            <!-- Actions Column -->
            <td class="px-9 py-4 flex space-x-4">
              <!-- Edit Link -->
              <router-link :to="{ name: 'admin-skills-edit-id', params: { id: skill.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <!-- Delete Action -->
              <a href="#" @click.prevent="deleteSkill(skill.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-skills-create' })"
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/skills?q=${search}&page=${page}`);
      return {
        filteredSkill: response.data.data,
  ...
  methods: {
    async fetchSkill() {
      try {
        const response = await this.$axios.$get(`/api/admin/skills?q=${this.search}`);
        this.filteredSkill = response.data.data;
    ...
    async deleteSkill(id) {
      try {
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/skills/${id}`);
          await this.fetchSkill(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/skills?q=${this.search}&page=${page}`);
        this.filteredSkill = response.data.data;
        this.pagination = response.data;
