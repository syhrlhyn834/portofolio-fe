<template>
  <div>
    <h1 class="text-2xl font-semibold mb-4 pt-4">Gallery</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
         ...
         <tbody>
          <tr v-for="gallery in filteredGallery" :key="gallery.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-gallery-edit-id', params: { id: gallery.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteGallery(gallery.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <nav class="flex items-center flex-wrap md:flex-row justify-between pt-4 md:pb-20" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
        Showing <span class="font-semibold text-gray-900 dark:text-white">{{ paginationGalleries.from }}</span> to <span class="font-semibold text-gray-900 dark:text-white">{{ paginationGalleries.to }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ paginationGalleries.total }}</span>
      </span>
      <ul class="inline-flex -space-x-px text-sm h-8">
        <li>
          <button @click.prevent="changePageGalleries(paginationGalleries.current_page - 1)" :disabled="!paginationGalleries.prev_page_url" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</button>
        </li>
        <li v-for="page in paginationGalleries.last_page" :key="page">
          <button @click.prevent="changePageGalleries(page)" :class="{'text-blue-600 bg-blue-50 dark:bg-gray-700 dark:text-white': page === paginationGalleries.current_page, 'text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white': page !== paginationGalleries.current_page}" class="flex items-center justify-center px-3 h-8 leading-tight border">
            {{ page }}
          </button>
        </li>
        <li>
          <button @click.prevent="changePageGalleries(paginationGalleries.current_page + 1)" :disabled="!paginationGalleries.next_page_url" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</button>
        </li>
      </ul>
    </nav>
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr v-for="category in filteredCategories" :key="category.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td class="px-9 py-4 flex space-x-4">
                <router-link :to="{ name: 'admin-gallery-category-edit-id', params: { id: category.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
                  <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.304 4.844 16.156 6.696M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844-1.707-1.707 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z"/>
                  </svg>
                </router-link>
...
<div data-dial-init class="fixed end-6 bottom-6 group">
  ...
  >
        <ul class="text-sm text-gray-500 dark:text-gray-300">
            <li>
              <nuxt-link :to="{ name: 'admin-gallery-create' }" class="flex items-center px-5 py-2 border-b border-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 hover:text-gray-900 dark:hover:text-white dark:border-gray-600">
                    ...
                    <span class="text-sm font-medium">Tambah Gallery</span>
              </nuxt-link>
            </li>
            <li>
              <nuxt-link :to="{ name: 'admin-gallery-category-create' }" class="flex items-center px-5 py-2 border-b border-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 hover:text-gray-900 dark:hover:text-white dark:border-gray-600">
...
export default {
  ...
  try {
    const galleriesResponse = await $axios.$get(`/api/admin/galleries?q=${search}&page=${page}`);
    const categoriesResponse = await $axios.$get(`/api/admin/categories?q=${search}&type=galleries&page=${page}`);
    return {
      filteredGallery: galleriesResponse.data.data,
      filteredCategories: categoriesResponse.data.data,
      paginationGalleries: galleriesResponse.data,
...
methods: {
  ...
  async fetchGallery() {
    try {
      const response = await this.$axios.$get(`/api/admin/galleries?q=${this.search}`);
      this.filteredGallery = response.data.data;
      this.paginationGalleries = response.data;
    } catch (error) {
  ...
  async deleteGallery(id) {
    try {
      ...
      if (confirmed.isConfirmed) {
        await this.$axios.delete(`/api/admin/galleries/${id}`);
        await this.fetchGallery(); // Refresh data after deletion
  ...
  async changePageGalleries(page) {
    try {
      const response = await this.$axios.$get(`/api/admin/galleries?q=${this.search}&page=${page}`);
      this.filteredGallery = response.data.data;
      this.paginationGalleries = response.data;
  ...
  async fetchCategory() {
    try {
      const response = await this.$axios.$get(`/api/admin/categories?q=${this.search}&type=galleries`);
  ...
  async changePageCategories(page) {
    try {
      const response = await this.$axios.$get(`/api/admin/categories?q=${this.search}&type=galleries&page=${page}`);
      this.filteredCategories = response.data.data;
      this.paginationCategories = response.data;
      this.$router.push({ path: this.$route.path, query: { q: this.search, page: page } });
    } catch (error) {
