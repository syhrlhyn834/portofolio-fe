<template>
  <div class="container mx-auto px-4">
    <Title :pageTitle="'Edit Gallery'" />
    <h1 class="text-2xl font-semibold mb-4">Edit Gallery</h1>
      ...
      <div class="mt-10">
        ...
        <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10"
          ...
          <div class="text-center">
            ...
            <p class="text-xs leading-5 text-gray-600">PNG, JPG, JPEG, SVG, MP4, MOV, etc. up to 10MB</p>
            <div v-if="filePreview">
              <template v-if="isImagePreview">
                <img :src="filePreview" alt="File Preview" class="mt-4 h-80 w-120 object-cover" />
              </template>
              <template v-else>
                <video controls :src="filePreview" class="mt-4 h-80 w-120 object-cover"></video>
              </template>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      gallery: {
        file: '', // Rename 'image' to 'file' to accommodate both images and videos
        title: '',
        category_id: '',
        content: '',
        description: ''
  ...
  async mounted() {
    try {
      ...
      const galleryResponse = await this.$axios.get(`/api/admin/galleries/${this.$route.params.id}`);
      const data = galleryResponse.data.data;
      this.gallery.title = data.title;
      this.gallery.category_id = data.category_id;
      this.gallery.content = data.content;
      this.gallery.description = data.description;

      // Check if the loaded file is an image or video
      this.isImagePreview = data.image ? data.image.match(/\.(jpeg|jpg|gif|png|svg)$/) : false;
      this.filePreview = this.isImagePreview ? data.image : data.video; // Adjust based on your API response structure
    } catch (error) {
  ...
  methods: {
    ...
    async updateGallery() {
      ...
      formData.append('file', this.gallery.file);
      ...
      try {
        await this.$axios.post(`/api/admin/galleries/${this.$route.params.id}`, formData);
        this.$swal.fire({
          title: 'BERHASIL!',
