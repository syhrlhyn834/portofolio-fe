<template>
  <div>
    ...
    <h1 class="text-2xl font-semibold mb-4">Education</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="education in filteredEducation" :key="education.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ education.title }}</td>
            <td class="px-6 py-4">{{ education.place }}</td>
            <td class="px-6 py-4">{{ education.year }}</td>
            <td class="px-6 py-4">{{ education.city }}</td>
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-education-edit-id', params: { id: education.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteEducation(education.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-education-create' })"
  ...
  >
    ...
    <span class="sr-only">Add Education</span>
  </button>
</div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/educations?q=${search}&page=${page}`);
      return {
        filteredEducation: response.data.data,
  ...
  methods: {
    async fetchEducation() {
      try {
        const response = await this.$axios.$get(`/api/admin/educations?q=${this.search}`);
        this.filteredEducation = response.data.data;
    ...
    async deleteEducation(id) {
      try {
        const confirmed = await this.$swal.fire({
          title: 'Are you sure?',
          text: 'You are about to delete this Education.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/educations/${id}`);
          await this.fetchEducation(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/educations?q=${this.search}&page=${page}`);
        this.filteredEducation = response.data.data;
        this.pagination = response.data;
