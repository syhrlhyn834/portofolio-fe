<template>
  <div>
    ...
    <h1 class="text-2xl font-semibold mb-4">Experience</h1>
      ...
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" class="px-6 py-3">Judul Experience</th>
        ...
        <tbody>
          <tr v-for="experience in filteredExperience" :key="experience.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ experience.id }}</td>
            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ experience.title }}</td>
            <td class="px-6 py-4">{{ experience.place }}</td>
            <td class="px-6 py-4">{{ experience.year }}</td>
            <td class="px-6 py-4">{{ experience.city }}</td>
            <td class="px-6 py-4">{{ experience.url }}</td>
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-experience-edit-id', params: { id: experience.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteExperience(experience.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <button
    type="button"
    @click="$router.push({ name: 'admin-experience-create' })"
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/experiences?q=${search}&page=${page}`);
      return {
        filteredExperience: response.data.data,
  ...
  methods: {
    async fetchExperience() {
      try {
        const response = await this.$axios.$get(`/api/admin/experiences?q=${this.search}`);
        this.filteredExperience = response.data.data;
    ...
    async deleteExperience(id) {
      try {
        const confirmed = await this.$swal.fire({
          title: 'Are you sure?',
          text: 'You are about to delete this Experience.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/experiences/${id}`);
          await this.fetchExperience(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/experiences?q=${this.search}&page=${page}`);
        this.filteredExperience = response.data.data;
        this.pagination = response.data;
