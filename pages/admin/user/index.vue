        ...
        <tbody>
          <tr v-for="user in filteredUsers" :key="user.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-9 py-4 flex space-x-4">
              <router-link :to="{ name: 'admin-user-edit-id', params: { id: user.id } }" class="text-blue-600 dark:text-blue-500 hover:underline">
              ...
              <a href="#" @click.prevent="deleteUser(user.id)" class="text-red-600 dark:text-red-500 hover:underline">
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
  ...
  async asyncData({ $axios, query }) {
    ...
    try {
      const response = await $axios.$get(`/api/admin/users?q=${search}&page=${page}`);
      return {
  ...
  methods: {
    async fetchUsers() {
      try {
        const response = await this.$axios.$get(`/api/admin/users?q=${this.search}`);
    ...
    async deleteUser(id) {
      try {
        ...
        if (confirmed.isConfirmed) {
          await this.$axios.delete(`/api/admin/users/${id}`);
          await this.fetchUsers(); // Refresh data after deletion
    ...
    async changePage(page) {
      try {
        const response = await this.$axios.$get(`/api/admin/users?q=${this.search}&page=${page}`);
        this.filteredUsers = response.data.data;
        this.pagination = response.data;
